...
local function tradeOres(pl)
	...
	for k, v in next, Ores.__R do
		local ret = hook.Run("CanPlayerTradeOreRarity", pl, k)
...
function Ores.TakePlayerPoints(ply, amount)
	assert(ply and ply:IsPlayer(), "[Ores] First argument is not a player")
	assert(isnumber(amount) and amount > 0, "[Ores] Amount argument is invalid, it must be a positive number")

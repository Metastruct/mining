...
local function DrawTitleText(txt,x,y,font,shadowColor,color,dist)
	local txtW = surface.GetTextSize(font)
...
local function DrawOutlinedRect(x,y,w,h,outlineColor,color)
	...
	local x,y,w,h = math.Round(x),math.Round(y),math.Round(w),math.Round(h)
...
local msa = Material("msa/msa_logo_1_unlitgeneric_2")
msa:SetFloat("$color",1)
msa:SetFloat("$color2",1)
msa:SetFloat("$alpha",0.033)
msa:SetInt("$additive",1)
msa:Recompute()
...
function PANEL:Init()
	...
	self.btnClose.Paint = Paint.DButton
	...
	self.TopPanel.Paint = function(_,w,h)
		...
		surface.SetMaterial(msa)
		local w = 256
		surface.DrawTexturedRect(self.TopPanel:GetWide()*0.5-w*0.5,self.TopPanel:GetTall()*0.5-w*0.5,w,w)
					...
					btn.DoClick = function(_)
						...
						if level >= 50 then
							...
							timer.Simple(5,function()
								if congrats:IsPlaying() then congrats:FadeOut(10) end
							end)
						else

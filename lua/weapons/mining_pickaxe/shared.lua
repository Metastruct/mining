...
function SWEP:Deploy()
	if not (ms and ms.Ores) then
		if SERVER then
			self:GetOwner():ChatPrint("Mining is not available on this server.")
			self:Remove()
		end
...
function SWEP:PrimaryAttack()
	...
	if tr.Hit then
		...
		if tr.Entity:IsValid() and not self.IgnoredEnts[tr.Entity:GetClass()] then
			if SERVER then
				self:DoDamage(tr)
			end
		elseif self.Stats.ShockwaveRange and self.Stats.ShockwaveRange > 0 then
			if IsFirstTimePredicted() then
				...
				-- Coal spawn
				if SERVER and tr.MatType == MAT_CONCRETE and math.random() <= 0.00075 then
					ms.Ores.GivePlayerOre(owner, 0, 1)
				end
			end
		end
	else
